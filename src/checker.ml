...
let precompute model =
  ...
  (*  let count = Array1.create int_elt c_layout num_states in *)
  let rec cache f =
    try
    ...
    with Not_found ->
	 ...
	 T -> Array2.fill slice valTrue
	  ...
	  for state = 0 to num_states - 1 do
       ...
       | Af f1 ->
	  Util.fail "Af unsupported"
(*	  let a1 = cache f1 in
	  for point = 0 to num_points - 1 do
	    for state = 0 to num_states - 1 do
	      let accum = ref [] in		  
	      if Array2.get a1 state point
	      then (Array1.set count state 0; accum := state :: (!accum))
	      else Array1.set count state (Model.Graph.out_degree kripke state)
	    done
	  done *)
       | Ex f1 ->
