...
type fsyn =
  ...
  | STATCMP of (string * fsyn * fsyn * float * string * float * float * float * int)
  (* atom fmla fmla radius operator threshold bin_min bin_max nbins *)
...
let rec formula_of_fsyn env f =
  ...
  | STATCMP (p,f1,f,rad,op,thr,min,max,nbins) -> Threshold (op,thr,Statcmp (p,formula_of_fsyn env f1,formula_of_fsyn env f,rad,min,max,nbins))
